...
export default function Anim1D() {
  ...
  return (
    <div className="grid md:grid-cols-[1fr_220px] min-h-95">
          ...
          {model === "cs" && (
            ...
            >
              ...
              <motion.circle
                ...
                fill="#1E293B"
                stroke={csPhase === "respond" ? "#7C3AED" : "#334155"}
                strokeWidth={csPhase === "respond" ? 3 : 2}
                animate={{ stroke: csPhase === "respond" ? "#7C3AED" : "#334155" }}
              ...
              {P2P_EDGES.map(([a, b]) => {
                ...
                return (
                  <motion.line key={`${a}-${b}`}
                    ...
                    animate={{ stroke: active ? "#7C3AED" : "#1E293B", opacity: active ? 0.9 : 0.35 }}
                ...
                return (
                  <g key={p.id}>
                    <motion.circle
                      ...
                      animate={{ stroke: active ? "#7C3AED" : "#334155" }}
      ...
      {/* ── info panel ── */}
      <div className="bg-white border-l border-gray-200 p-5 flex flex-col gap-4">
          ...
          {(["cs", "p2p"] as Model[]).map(m => (
            <button key={m} onClick={() => setModel(m)}
              className={`flex-1 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                model === m ? "bg-white text-violet-700 shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
            >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div>
              <h3 className="font-sora font-bold text-gray-900 text-sm">{info.name}</h3>
            ...
            <div className="border-t border-gray-100 pt-3">
              ...
              <p className="text-xs text-violet-600 font-mono">{info.examples}</p>
            </div>

            {model === "cs" && (
              <div className="bg-violet-50 rounded-xl px-4 py-3 mt-auto">
                <p className="text-[10px] font-bold text-violet-400 uppercase tracking-wider">Requests served</p>
                <motion.p
                  key={reqCount}
