  ...
  return (
    <div className="bg-[#0F172A] p-5 flex flex-col gap-4 min-h-[420px]">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] text-gray-400">Send HTTP request from:</span>
        {PRIVATE_HOSTS.map(h => (
          <button
            key={h.id}
        ...
        <button
          onClick={reset}
          className="flex items-center gap-1 ml-auto px-3 py-1.5 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs border border-[#334155] transition-colors"
        >
      ...
      <div className="flex items-stretch gap-3 flex-1">
        ...
        <div className="flex flex-col justify-around gap-2 w-32 shrink-0">
          <p className="text-[8px] text-gray-500 font-bold uppercase tracking-wider text-center">Private LAN</p>
          {PRIVATE_HOSTS.map(h => (
            <div
              key={h.id}
            ...
            >
              ...
              <p className="text-[8px] font-mono text-gray-400">{h.ip}</p>
              {done.includes(h.id) && (
        ...
        <div className="flex flex-col items-center justify-center gap-1.5 w-28 shrink-0">
          <div className="w-full rounded-xl bg-[#1E293B] border-2 border-[#334155] p-2 text-center">
            <p className="text-[9px] font-bold text-gray-300">Router / NAT</p>
            <p className="text-[8px] font-mono text-violet-300">{PUBLIC_IP}</p>
          </div>
        ...
        <div className="flex flex-col justify-center gap-2 flex-1">
          <p className="text-[8px] text-gray-500 font-bold uppercase tracking-wider text-center">Internet</p>
          <div className="rounded-xl bg-[#1E293B] border border-[#334155] p-3 text-center">
            <p className="text-[9px] font-bold text-gray-300">Remote Server</p>
            <p className="text-[8px] font-mono text-amber-300">{SERVER_IP}</p>
            <p className="text-[7px] text-gray-500 mt-0.5">port 80</p>
          </div>
          {selectedEntry && (
            <motion.div
              initial={{ opacity: 0, y: 6 }}
              animate={{ opacity: 1, y: 0 }}
              className="rounded-xl bg-[#1E293B] border border-violet-500/30 p-2.5 text-[8px]"
            >
              <p className="font-bold text-violet-400 mb-1.5">Header rewrite (before/after)</p>
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-1">
                  <span className="text-[7px] text-gray-500 w-12 shrink-0">Src IP</span>
                ...
                <div className="flex items-center gap-1">
                  <span className="text-[7px] text-gray-500 w-12 shrink-0">Src Port</span>
              ...
              {natTable.map(e => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono text-gray-400">{e.destIP}:80</span>
