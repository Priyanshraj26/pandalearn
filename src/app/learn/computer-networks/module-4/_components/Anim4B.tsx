    ...
    <div className="bg-[#0F172A] p-5 flex flex-col gap-4 min-h-[420px]">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="flex flex-col gap-1">
          <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Network block</label>
          <input
            value={input}
            onChange={e => handleInput(e.target.value)}
            className="bg-[#1E293B] border border-[#334155] rounded-lg px-3 py-1.5 text-xs font-mono text-gray-200 outline-none focus:border-violet-500 w-44"
          />
        ...
        <label className="flex items-center gap-1.5 cursor-pointer">
          <input type="checkbox" checked={useCase} onChange={e => setUseCase(e.target.checked)}
            className="accent-violet-500" />
          <span className="text-[10px] text-gray-400">Use-case labels</span>
        </label>
      </div>
      ...
      {subnets.length > 0 ? (
        <div className="flex flex-col gap-2">
          <p className="text-[9px] text-gray-500 uppercase font-bold tracking-wider">Click a subnet to inspect</p>
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                <div key={label}>
                  <p className="text-[8px] text-gray-500 font-bold uppercase tracking-wider mb-0.5">{label}</p>
      ...
      {parsed && subnets.length > 0 && (
        <div className="flex flex-wrap gap-3 text-[10px] text-gray-400 border-t border-[#334155] pt-3">
          <span>Base: <span className="text-violet-300 font-mono">{input}</span></span>
          <span>Borrowed: <span className="text-amber-300">{borrow} bits</span></span>
          <span>New prefix: <span className="text-violet-300 font-mono">/{newPrefix}</span></span>
