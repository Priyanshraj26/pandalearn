    ...
    <div className="bg-[#0F172A] p-5 flex flex-col gap-4 min-h-[440px]">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex flex-col gap-0.5">
          <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Destination</label>
          <select
            value={dest}
            onChange={e => { setDest(e.target.value); reset() }}
            className="bg-[#1E293B] border border-[#334155] rounded-lg px-2 py-1.5 text-xs font-mono text-gray-200 outline-none focus:border-violet-500"
          >
        ...
        <button
          disabled={running}
          onClick={traceRoute}
          className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold transition-colors self-end"
        >
          Trace Route
        </button>
        <button
          onClick={breakLink}
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors self-end ${brokenLink ? "bg-red-700 hover:bg-red-600 text-white" : "bg-[#1E293B] hover:bg-[#334155] text-gray-400 border border-[#334155]"}`}
        >
          {brokenLink ? "Restore Link" : "Break R2–R4"}
        </button>
        <button
          onClick={reset}
          className="px-3 py-1.5 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs border border-[#334155] transition-colors self-end"
        >
          <RotateCcw size={10} />
        </button>
        <button
          onClick={() => setShowAdd(s => !s)}
          className="flex items-center gap-1 px-3 py-1.5 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs border border-[#334155] transition-colors self-end"
        >
      ...
      {showAdd && (
        <div className="flex items-center gap-2 bg-[#1E293B] p-3 rounded-xl border border-[#334155] text-[10px] text-gray-400">
          <span>Injecting a static route would redirect traffic. In a real router: <code className="text-violet-300">ip route add 10.0.3.0/24 via 10.0.1.2</code></span>
          <button onClick={() => setShowAdd(false)} className="ml-auto text-gray-500 hover:text-gray-300"><X size={12} /></button>
        </div>
      )}
        ...
        <div className="flex-1 relative" style={{ minWidth: 0 }}>
          <svg viewBox="0 0 480 220" className="w-full h-full" style={{ minHeight: 180 }}>
            ...
            {LINKS.map(l => {
              ...
              const isActive = path.length > 1 && path.some((r, i) =>
                i < path.length - 1 &&
          ...
          <div className="bg-[#1E293B] rounded-xl border border-[#334155] overflow-hidden flex-1">
            {activeRouter ? (
              <div className="overflow-auto">
                <div className="grid grid-cols-2 text-[7px] text-gray-500 font-bold px-2 py-1.5 border-b border-[#334155] bg-[#0F172A]">
                ...
                {ROUTING_TABLES[activeRouter].map((row, i) => (
                  ...
                  >
                    <span className={`font-mono ${i === activeRowIdx ? "text-violet-300 font-bold" : "text-gray-400"}`}>{row.dest}</span>
                    <span className={`font-mono ${i === activeRowIdx ? "text-amber-300 font-bold" : "text-gray-500"}`}>{row.nextHop}</span>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-[9px] text-gray-600 p-3 italic">Start a trace to see live table lookups</p>
            )}
          </div>
          {done && (
            <motion.div
