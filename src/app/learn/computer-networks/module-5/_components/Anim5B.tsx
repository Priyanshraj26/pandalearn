  ...
  return (
    <div className="bg-[#0F172A] p-5 flex flex-col gap-5 min-h-[380px]">
      ...
      <div className="flex items-center gap-3">
        <span className="text-[10px] text-gray-400 w-24 shrink-0">Window size: {bufferFull ? "0 (zero)" : windowSize}</span>
        <input
          ...
          disabled={bufferFull}
        />
        <span className="text-[10px] text-gray-500 w-16 text-right">{TOTAL_BYTES - acked - sent} bytes free</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold transition-colors"
        >
          Send Window →
        </button>
        <button
      ...
      <AnimatePresence>
        ...
        {probing && (
          <motion.div
            initial={{ opacity: 0, y: 4 }}
            animate={{ opacity: 1, y: 0 }}
            className="flex items-center gap-2 px-3 py-2 bg-violet-900/30 border border-violet-700/40 rounded-xl"
          >
            <motion.div
      ...
      {/* stats */}
      <div className="flex gap-4 text-[10px] text-gray-500 border-t border-[#334155] pt-3">
        <span>ACK'd: <span className="text-gray-300 font-mono">{acked}</span></span>
        <span>In-flight: <span className="text-violet-300 font-mono">{sent}</span></span>
        <span>Window: <span className="text-blue-300 font-mono">{bufferFull ? 0 : windowSize}</span></span>
        <span>Remaining: <span className="text-gray-300 font-mono">{TOTAL_BYTES - acked}</span></span>
      </div>
    </div>
  )
}
