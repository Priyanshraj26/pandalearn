    ...
    <div className="bg-[#0F172A] p-5 flex flex-col gap-4 min-h-[400px]">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          disabled={running}
          onClick={startSim}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold transition-colors ml-auto"
        >
          <Play size={10} /> Simulate
        </button>
        <button
          onClick={reset}
          className="flex items-center gap-1 px-3 py-1.5 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs border border-[#334155] transition-colors"
        >
          <RotateCcw size={10} />
        </button>
      </div>
      ...
      <div className="flex-1 flex gap-4">
        ...
        <div className="w-40 shrink-0 flex flex-col gap-3">
          <div className="bg-[#1E293B] rounded-xl p-3 border border-[#334155] space-y-2">
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Legend</p>
            {(["tahoe","reno","cubic"] as Algorithm[]).map(a => (
              <div key={a} className="flex items-center gap-1.5">
                <div className="w-4 h-0.5" style={{ backgroundColor: ALGO_COLORS[a] }} />
                <span className="text-[9px] text-gray-400">TCP {a.charAt(0).toUpperCase() + a.slice(1)}</span>
              </div>
            ))}
            <div className="flex items-center gap-1.5 pt-1 border-t border-[#334155]">
              <Zap size={9} className="text-red-400" />
              <span className="text-[9px] text-gray-400">3-dup-ACK (RTT 12)</span>
            </div>
            <div className="flex items-center gap-1.5">
              <AlertTriangle size={9} className="text-orange-400" />
              <span className="text-[9px] text-gray-400">Timeout (RTT 28)</span>
            </div>
          </div>

          <div className="bg-[#1E293B] rounded-xl p-3 border border-[#334155] text-[9px] text-gray-400 space-y-2">
            <p className="font-bold text-gray-300">On 3-dup-ACK:</p>
            <p><span className="text-red-400">Tahoe:</span> cwnd→1, slow start</p>
            <p><span className="text-violet-400">Reno:</span> cwnd→ssthresh, fast recovery</p>
            <p><span className="text-emerald-400">CUBIC:</span> concave growth, less drastic cut</p>
            <p className="border-t border-[#334155] pt-2 font-bold text-gray-300">On Timeout:</p>
