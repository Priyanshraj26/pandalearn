    ...
    <div className="bg-[#0F172A] p-5 min-h-[400px] flex flex-col gap-5">
      ...
      <div className="flex items-center gap-2">
        ...
        {phase === 0 ? (
          <button
            onClick={start}
            className="flex items-center gap-1.5 px-3 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-semibold transition-colors shrink-0"
          >
          ...
          <button
            onClick={reset}
            className="flex items-center gap-1.5 px-3 py-2 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs transition-colors border border-[#334155] shrink-0"
          >
      ...
      <div className="flex gap-3 flex-1">
        ...
        <div className="flex flex-col items-center gap-1.5 w-16 shrink-0 pt-1">
          ...
          >
            <Monitor size={22} className={running ? "text-violet-300" : "text-gray-500"} />
          </motion.div>
          <span className="text-[9px] text-gray-500 font-medium">Browser</span>
          <span className="text-[9px] text-gray-600 font-mono">192.168.1.10</span>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-2.5 justify-start pt-1 min-w-0">
            ...
            {STEPS.slice(0, phase).map((s, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 px-0.5">
                  ...
                  <span className="text-[9px] text-gray-500 truncate">{s.info}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          {running && phase >= 8 && phase < STEPS.length && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex items-center gap-2 text-[9px] text-gray-500"
            >
              <motion.div
        ...
        <div className="flex flex-col items-center gap-1.5 w-16 shrink-0 pt-1">
          <motion.div
            ...
            className="w-14 h-14 rounded-xl border-2 flex items-center justify-center"
          >
            <Server size={22} className={serverLit ? "text-emerald-300" : "text-gray-500"} />
          </motion.div>
          <span className="text-[9px] text-gray-500 font-medium">Server</span>
          <span className="text-[9px] text-gray-600 font-mono">203.0.113.42</span>
        </div>
      </div>
    </div>
  )
