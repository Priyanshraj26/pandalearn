    ...
    <div className="bg-[#0F172A] p-5 flex flex-col gap-5 min-h-[460px]">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] text-gray-400">Posts to fetch: <span className="text-violet-300 font-mono">{numPosts}</span></label>
          <input type="range" min={1} max={10} value={numPosts} onChange={e => setNumPosts(+e.target.value)} className="accent-violet-500" />
        </div>
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] text-gray-400">User fields needed: <span className="text-violet-300 font-mono">{numFields}</span></label>
          <input type="range" min={1} max={ALL_USER_FIELDS.length} value={numFields} onChange={e => setNumFields(+e.target.value)} className="accent-violet-500" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-300 font-sora">REST</span>
            <span className="text-[9px] text-gray-500">2 requests</span>
          </div>
          ...
          <div className="bg-[#1E293B] rounded-xl border border-blue-800/40 p-3 text-[9px] font-mono">
            ...
            <div className="text-gray-500 mb-2">→ returns all {ALL_USER_FIELDS.length} fields:</div>
            ...
            <div className="mt-2 text-gray-600">
              <span className="text-gray-500">{ALL_USER_FIELDS.length - numFields} fields</span> unused / over-fetched
            </div>
          </div>
          ...
          <div className="bg-[#1E293B] rounded-xl border border-blue-800/40 p-3 text-[9px] font-mono">
            ...
            <div className="text-gray-500 mb-2">→ returns {numPosts} posts × {ALL_POST_FIELDS.length} fields:</div>
            <div className="flex flex-wrap gap-1">
            ...
            <div className="mt-2 text-gray-600">
              <span className="text-gray-500">{(ALL_POST_FIELDS.length - 1) * numPosts} fields</span> per {numPosts} posts unused
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-violet-300 font-sora">GraphQL</span>
            <span className="text-[9px] text-gray-500">1 request</span>
          </div>

          {/* Single query */}
          <div className="bg-[#1E293B] rounded-xl border border-violet-800/40 p-3 text-[9px] font-mono flex-1">
            <div className="text-violet-400 mb-1">POST /graphql</div>
            <pre className="text-gray-300 text-[8px] leading-relaxed whitespace-pre-wrap">{`query {
  user(id: 42) {
${neededUserFields.map(f => `    ${f}`).join("\n")}
    posts(last: ${numPosts}) {
      title
            ...
            <div className="mt-2 border-t border-[#334155] pt-2 text-gray-500">
              Response: exactly <span className="text-violet-300">{numFields + numPosts}</span> fields
            </div>
          </div>

          <motion.div
            animate={{ width: "100%" }}
            className="flex items-center justify-between px-3 py-2 bg-violet-900/20 border border-violet-700/30 rounded-xl"
          >
            <span className="text-[10px] text-violet-400">Total transferred</span>
            <span className="text-xs font-bold font-mono text-violet-300">~{gqlTotal} B</span>
          </motion.div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-[#334155]">
        <span className="text-[10px] text-gray-500">GraphQL savings:</span>
