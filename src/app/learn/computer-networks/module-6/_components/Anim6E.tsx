...
const WS_PUSH_INTERVAL = 1 // push every second
    ...
    <div className="bg-[#0F172A] p-5 flex flex-col gap-4 min-h-[440px]">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button disabled={running} onClick={start}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold transition-colors"
        >
          <Play size={10} /> Simulate 30 s
        </button>
        <button onClick={reset} className="flex items-center gap-1 px-3 py-1.5 bg-[#1E293B] hover:bg-[#334155] text-gray-400 rounded-lg text-xs border border-[#334155] transition-colors">
        ...
        {(running || done) && (
          <div className="ml-auto flex items-center gap-2 px-3 py-1 bg-[#1E293B] rounded-lg">
            <span className="text-[9px] text-gray-400">Live counter:</span>
            <motion.span
              key={counter}
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-300 font-sora">HTTP Polling</span>
            <span className="text-[9px] text-gray-500">every {POLL_INTERVAL}s</span>
          </div>
            ...
            <AnimatePresence initial={false}>
              {pollVisible.map(m => (
                ...
                >
                  ...
                  <span className="ml-auto text-[7px] font-mono text-gray-600">{m.bytes}B</span>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-violet-300 font-sora">WebSocket</span>
            <span className="text-[9px] text-gray-500">push only</span>
          </div>
          <div className="flex-1 bg-[#0A0F1A] rounded-xl border border-[#1E293B] p-2 flex flex-col gap-0.5 overflow-hidden min-h-[160px]">
            <AnimatePresence initial={false}>
              {wsVisible.map(m => (
          ...
          <div className="flex items-center justify-between px-2 py-1.5 bg-violet-900/20 border border-violet-700/30 rounded-xl">
            <span className="text-[9px] text-violet-400">Bytes sent</span>
            <span className="text-[10px] font-bold font-mono text-violet-300">{wsBytes.toLocaleString()} B</span>
          </div>
        </div>
      </div>
      ...
      {done && pollBytes > 0 && (
        ...
        >
          <div className="flex justify-between text-[9px] text-gray-400">
            ...
            <span className="font-mono text-gray-500">{wsBytes.toLocaleString()} B vs {pollBytes.toLocaleString()} B</span>
          </div>
          <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
            <div className="bg-violet-600 rounded-l" style={{ width: `${(wsBytes / pollBytes) * 100}%` }} />
            <div className="bg-emerald-700 flex-1 rounded-r" />
          </div>
          <div className="flex justify-between text-[8px] font-mono text-gray-600">
            <span className="text-violet-400">WS: {wsBytes.toLocaleString()} B</span>
