...
function SectionHeading({ n, title, icon }: { n: string; title: string; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      <span className="text-[11px] font-mono text-violet-500 bg-violet-950/60 border border-violet-800/40 px-2 py-1 rounded-lg">
        {n}
      </span>
      {icon && <span className="text-violet-400">{icon}</span>}
      <h2 className="text-xl font-bold text-white font-sora">{title}</h2>
    </div>
  )
}
    ...
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-20">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-semibold tracking-[0.16em] uppercase text-violet-500">
            Computer Networks · Module 6
          </span>
        </div>
        <h1 className="text-4xl font-bold text-white font-sora leading-tight">
